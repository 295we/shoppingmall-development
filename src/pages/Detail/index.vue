...
export default {
  ...
  computed: {
    ...
    // 给子组件的数据
    skuImageList() {
      // 如果服务器的数据没有及时回来，那么skuInfo对象是一个空对象
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    // 产品的售卖属性值切换高亮 排他思想，现将所有设为0，再将点击的设为1
    ...
    changeSkuNum(event) {
      ...
      if (isNaN(value) || value < 1 || value.toString().indexOf('.') != -1) {
        alert('输入的数字不合法');
        this.skuNum = 1;
      } else {
        this.skuNum = value;
    ...
    addShopCar() {
      // 如下的函数调用就是派发action，而action的函数使用了async，所以是一个Promise
      this.$store
        ...
        .then(result => {
          // 利用回话存储将skuInfo存在sessionStorage中
          this.$router.push({
            path: '/addcartsuccess',
            query: {
              skuNum: this.skuNum,
